...
def getNewAlbumsForArtist(artist_id):
    ...
    for item in albums['items']:
        ...
        try:
            if (len(uni_release_date) > 4):
                release_date = datetime.strptime(uni_release_date,'%Y-%m-%d').strftime('%Y/%m/%d')
            else :
                release_date = datetime.strptime(uni_release_date,'%Y').strftime('%Y/%m/%d')
...
def replaceTracks(tracks, playlistName, description):
    x = spotifyObject.user_playlist_replace_tracks(user_config['username'], user_config['weekly_playlist_uri_partial'], tracks)
...
def run(config):
    ...
    user_config = config
    artist_names = ['Juice WRLD', 'A$AP Rocky', 'XXXTentacion', 'Tyler, The Creator', 'Frank Ocean', 'Khalid', 'Logic', 'Billie Eilish', 'Trippie Redd']
    artist_names.extend(['Travis Scott', 'Kayne West', 'Kid Cudi', 'Lil Skies', 'Ski Mask The Slump God', 'Drake', 'Post Malone', 'Jaden Smith', 'The Weeknd'])
